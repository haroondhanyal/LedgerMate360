...
export class AuthPage{
  ...
  async login(email=env.email,password=env.password){const response=await this.page.request.post(`${env.apiUrl}/auth/login`,{data:{email,password}});expect(response.ok()).toBeTruthy();const session=await response.json();await this.open();await this.page.evaluate(value=>localStorage.setItem("lm_session",JSON.stringify(value)),session);await this.page.reload();await expect(shellLocators(this.page).profile).toBeVisible({timeout:20_000})}
